...
IGNORE_ALOHA_MINI_COLLISION_STRS = ["mat", "rug", "carpet"]
...
class ReplicaCADSceneBuilder(SceneBuilder):
    ...
    def initialize(self, env_idx: torch.Tensor):
        ...
        if robot_uid == "fetch":
            ...
            if hasattr(self.env.agent, 'keyframes') and 'rest' in self.env.agent.keyframes:
                self.env.agent.reset(self.env.agent.keyframes["rest"].qpos)
            # Position AlohaMini in the scene - slightly elevated for wheel radius
    ...
    def disable_aloha_mini_move_collisions(
        self,
        actor: Actor,
        disable_base_collisions=False,
    ):
        """Disable collisions between AlohaMini robot wheels and specified actor."""
        actor.set_collision_group_bit(
            group=2, bit_idx=ALOHA_MINI_WHEELS_COLLISION_BIT, bit=1
        )
        if disable_base_collisions:
            actor.set_collision_group_bit(
    ...
    def disable_robot_move_collisions(
        self,
        actor: Actor,
        disable_base_collisions=False,
    ):
        """Disable collisions between robot and specified actor based on robot type."""
        robot_uid = self.env.robot_uids
        if robot_uid == "fetch":
